...
function drawWheel() {
    ...
    segments.forEach((segment, i) => {
        ...
        // Slice
        g.moveTo(0, 0);
        g.arc(0, 0, WHEEL_RADIUS, startAngle, endAngle);
        g.lineTo(0, 0);
        g.fill({ color: segment.color });
        g.stroke({ width: 2, color: 0x0f0d1f, alpha: 1 }); // Background color to create gap

        // Inner decorative line (Inset with rounded corners)
        const inset = 4;
...
function createSpinButton() {
    ...
    glow.filterArea = app.screen; // optimization
